...
| `apps/web/src/main.ts` | `?shell=desktop` marker detection (tags `<html data-shell="desktop">`) + resolved-theme reporting to the shell preload | window integration — see above |
...
- The Windows caption-button overlay is solid and tracks the page theme only after the renderer's first report; until then (and if the page never loads) it shows the dark window surface.
