...
describe('web e2e: lifecycle & chrome (workspace flow / reload / dark mode)', () => {
  ...
  it('sends the first prompt from the empty-state hero (all modes)', async () => {
    ...
    if (MODE !== 'record') {
      // A preceding test may have left the pointer over a tooltip-bearing
      // control; move it onto the hero and wait for any transient tooltip to
      ...
      // Park the pointer outside the viewport and blur the active element so
      // the golden always captures the resting state regardless of which
      // trigger fired.
