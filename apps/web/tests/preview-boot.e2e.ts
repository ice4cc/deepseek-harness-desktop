...
async function bootPreview(origin: string, browser: Browser): Promise<void> {
  ...
  try {
    ...
    // Park the pointer in open conversation space before the next click: when
    // the document panel column is open (auto-followed file changes), "Load
    // earlier" sits beneath the catalog's coverage, and a direct approach from
    // the trigger re-hovers it — the reopened catalog then intercepts the click.
    await page.mouse.move(1000, 600)
    await expect.poll(() => catalog.count(), { timeout: 5_000 }).toBe(0)
