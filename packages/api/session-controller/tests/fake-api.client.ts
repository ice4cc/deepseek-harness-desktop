...
export class FakeApiClient {
  ...
  onWorkspaceReadTextFile: (payload: unknown) => Promise<RemoteResult<{ path: string; content: string; size: number; version: string }>> =
    payload => Promise.resolve(remoteOk({ path: (payload as { path: string }).path, content: '', size: 0, version: 'fake' }))

  onWorkspaceWriteTextFile: (payload: unknown) => Promise<RemoteResult<{ path: string; version: string; size: number }>> =
    payload => Promise.resolve(remoteOk({ path: (payload as { path: string }).path, version: 'fake', size: 0 }))
  ...
  sessionRemotes(): RuntimeRemotes {
    return {
      ...
      workspace: {
        ...
        readTextFile: payload => this.record('workspace.readTextFile', payload, this.onWorkspaceReadTextFile(payload)),
        writeTextFile: payload => this.record('workspace.writeTextFile', payload, this.onWorkspaceWriteTextFile(payload)),
