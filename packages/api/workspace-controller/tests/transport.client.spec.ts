...
class ScriptedWorkspaceRemote implements WorkspaceRemote {
  ...
  readTextFile(_request: TextFileReadRequest, _signal?: AbortSignal): Promise<RemoteResult<TextFileReadValue>> {
    throw new Error('unused')
  }

  writeTextFile(_request: TextFileWriteRequest, _signal?: AbortSignal): Promise<RemoteResult<TextFileWriteValue>> {
    throw new Error('unused')
  }
...
class CommandWorkspaceRemote implements WorkspaceRemote {
  ...
  readonly readTextFile = vi.fn<WorkspaceRemote['readTextFile']>(request => Promise.resolve(remoteOk({
    path: request.path, content: '', size: 0, version: 'fake',
  })))

  readonly writeTextFile = vi.fn<WorkspaceRemote['writeTextFile']>(request => Promise.resolve(remoteOk({
    path: request.path, version: 'fake', size: 0,
  })))
