...
class FakeWorkspaceRemote implements WorkspaceRemote {
  ...
  onReadTextFile: (request: TextFileReadRequest) => Promise<RemoteResult<TextFileReadValue>> = request =>
    Promise.resolve(remoteOk({ path: request.path, content: '', size: 0, version: 'fake' }))
  onWriteTextFile: (request: TextFileWriteRequest) => Promise<RemoteResult<TextFileWriteValue>> = request =>
    Promise.resolve(remoteOk({ path: request.path, version: 'fake', size: 0 }))
  ...
  readTextFile(request: TextFileReadRequest): Promise<RemoteResult<TextFileReadValue>> {
    this.record('readTextFile', request)
    return this.onReadTextFile(request)
  }

  writeTextFile(request: TextFileWriteRequest): Promise<RemoteResult<TextFileWriteValue>> {
    this.record('writeTextFile', request)
    return this.onWriteTextFile(request)
  }
