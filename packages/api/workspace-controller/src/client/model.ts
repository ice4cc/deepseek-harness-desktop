...
export class ClientWorkspaceModel implements WorkspaceFollowSink {
  ...
  /**
   * Read one text file through the Host document-editor verb.
   * @param path - fully qualified file path to read.
   * @returns generated Remote result.
   */
  async readTextFile(path: string): Promise<RemoteResult<TextFileReadValue>> {
    let result: RemoteResult<TextFileReadValue>
    try {
      result = await this.remote.readTextFile({ path })
    } catch (error) {
      result = failureResult(error)
    }
    return result
  ...
  /**
   * Write one text file's full content through the Host document-editor verb.
  ...
  ): Promise<RemoteResult<TextFileWriteValue>> {
    let result: RemoteResult<TextFileWriteValue>
    try {
      result = await this.remote.writeTextFile({
        path,
        content,
