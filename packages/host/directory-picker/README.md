...
Browse primitives fail with the typed `DirectoryPickerError` (`directory-unreadable` / `directory-exists` / `directory-create-failed`, each carrying the subject `path`), which the consuming gateway maps 1:1 onto wire error codes. `DirectoryEntry` rows carry a host-owned `hidden` flag (POSIX dot convention) so display policy stays client-side; `DirectoryListing.crumbs` is the ancestor chain from the filesystem root, every crumb a jump target. The seam also hosts the two pure helpers every host consumer of its primitives shares: `fullyQualified(path)` — the wire-path fence that rejects forms `resolve()` would rebase under the process cwd or current drive — and `raceAbort(operation, signal)`, which rejects with the caller's abort reason the moment it fires and swallows the abandoned operation's late settlement. Design rationale, the `ctx.fs` separation, and the policy decisions live in [the directory-picker capability seam Agent Note](../../../.agents/notes/implemented/architecture/2026-07-28-directory-picker-capability-seam.md).
