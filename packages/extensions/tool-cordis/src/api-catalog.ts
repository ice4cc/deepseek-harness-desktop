...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: '@Remote(\'readTextFile\') readTextFile(request: TextFileReadRequest, signal: AbortSignal): Promise<TextFileReadValue>',
        description: 'Read one text file for the in-app document editor, serving the host filesystem directly under every composed picking kind.',
        parameters: [{ name: 'request', description: 'fully qualified file path to read.' }, { name: 'signal', description: 'caller lifetime; abort stops the read instead of outliving it.' }],
        returns: 'the decoded content with its size and freshness token.',
      },
      {
        signature: '@Remote(\'writeTextFile\') writeTextFile(request: TextFileWriteRequest, signal: AbortSignal): Promise<TextFileWriteValue>',
        description: 'Write one text file\'s full content (the document editor\'s save), guarded by an optional freshness token so a stale save never clobbers a moved file.',
        parameters: [{ name: 'request', description: 'fully qualified target, full content, and optional guard.' }, { name: 'signal', description: 'caller lifetime; abort stops the write instead of outliving it.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'TextFileReadRequest',
    declaration: 'export interface TextFileReadRequest {\n    readonly path: string;\n}',
  },
  {
  ...
  {
    name: 'TextFileWriteRequest',
    declaration: 'export interface TextFileWriteRequest {\n    readonly path: string;\n    readonly content: string;\n    readonly expectedVersion?: string;\n}',
  },
  {
    name: 'TextFileWriteValue',
    declaration: 'export interface TextFileWriteValue {\n    readonly path: string;\n    readonly version: string;\n    readonly size: number;\n}',
  },
  {
