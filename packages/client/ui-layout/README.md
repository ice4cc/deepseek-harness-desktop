...
This package provides the shell layout of the Web GUI: a four-column AppFrame with resizable sidebar, document-panel, and details columns, a concession chain that shrinks details and then the document panel (auto-closing each when space runs out), and the `ctx.layout` panel-geometry service other plugins call to open or close the document panel or details column. It also seats the theme presenter, which projects the resolved color scheme, alias tokens, content font size, and `theme-color` metadata onto the document. Choose it for the standard window chrome; panel geometry is transient and resets on reload.
...
Mount this plugin at the root slot; it then renders the app frame around whatever occupies the sidebar, conversation, document-panel, and details columns. Users resize the sidebar by dragging its invisible hit strip and the document panel or details panel by dragging their floating pills; when the window narrows, details shrinks and auto-closes first, then the document panel does the same. A closed sidebar retains a 56px control rail; the document panel and details close to zero width.
...
- [ui-doc-panel](../ui-doc-panel/README.md) — occupies the `docPanel` column.
...
- **Panel geometry is transient** — reload restores the sidebar default with the document panel and details closed; switching between distinct Session ids also closes details and forgets its dragged width, while unselected surfaces render details at zero width without modifying geometry.
- **Concession-chain auto-close derives a zero width without touching the preferred width** — details concedes first, then the document panel; each restores itself when the window widens; consumers must not read the stored panel widths as the rendered truth.
