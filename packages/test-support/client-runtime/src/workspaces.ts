...
import type {
  IWorkspaces, TextFileReadValue, TextFileWriteValue, WorkspaceId, WorkspaceSnapshot, WorkspaceView,
...
export class TestWorkspaces implements IWorkspaces {
  ...
  /**
   * Read one text file (recorded). The default echoes an empty read; stub for
   * content or failure flows.
  ...
  async readTextFile(path: string): Promise<TextFileReadValue> {
    ...
    if (stub !== undefined) return await (stub(path) as Promise<TextFileReadValue>)
    return { path, content: '', size: 0, version: `test-${path}` }
  }

  /**
   * Write one text file's full content (recorded). The default echoes a fresh
   * baseline; stub for failure flows.
  ...
  async writeTextFile(path: string, content: string, expectedVersion?: string): Promise<TextFileWriteValue> {
    ...
    if (stub !== undefined) return await (stub(path, content, expectedVersion) as Promise<TextFileWriteValue>)
    return { path, version: `test-${path}`, size: new TextEncoder().encode(content).length }
  }
}
