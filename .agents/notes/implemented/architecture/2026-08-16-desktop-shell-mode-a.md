...
- The desktop GUI *is* the web app: deep UI customization goes through the existing client-plugin and slot seams (Phase 2), not a parallel codebase; upstream sync touches only three upstream files (`pnpm-workspace.yaml` allowBuilds, two `tsconfig.base.json` paths entries for the picker packages — an upstream gap this work exposed — and root `package.json` scripts), all listed in `apps/desktop/README.md`.
