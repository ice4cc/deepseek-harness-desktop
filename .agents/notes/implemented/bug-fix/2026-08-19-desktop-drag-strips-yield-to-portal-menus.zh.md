...
桌面壳里，会话列的空旷区域（空态 hero 面板，或打开会话后会话头上方的 44 px 条带）与侧栏品牌行是窗口拖拽条（[桌面壳说明](../architecture/2026-08-16-desktop-shell-mode-a.md)）。Chromium 在浏览器进程中缓存拖拽矩形，只要 mousedown 落在其内就路由给窗口——即使有其他元素绘制在上方；而另一棵树的元素无法裁出条带自己的缓存矩形——这正是设置对话框在给自己的层声明 no-drag 之外，还需要 `<html data-settings-open>` 标记加各条带让位的原因。
